...
@queue_router.post("/", summary="Создание (пересоздание) очереди из списка id треков, допустимы дубликаты")
...
@queue_router.get("/", summary="Получение очереди треков")
async def queue_get(
    current_user: dict = Depends(get_current_user)):
    return queue.get_queue(_user_id(current_user))


@queue_router.post("/tracks", summary="Добавить треки в конец очереди")
...
@queue_router.post("/tracks/next", summary="Вставляет трек после указанной позиции очереди")
...
@queue_router.delete("/tracks/{position}", summary="Убрать трек из очереди по позиции")
...
@queue_router.delete("/", summary="Очистить очередь")
